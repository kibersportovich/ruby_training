...
|     | |  0  | |     |
------- ------- -------
Prior to the development of T9 systems (predictive text entry), the method to type words was called "multi-tap" and involved pressing a button repeatedly to cycle through all its possible values, in order. 

For example:

Pressing the button 7 repeatedly will cycle through the letters 
P -> Q -> R -> S -> 7 -> P -> ....
...
A character is "locked in" and inserted into the message once the user presses a different key or pauses for a short period of time 
...
To type in the message "ABC", you would press the button 2 once, wait a second, then press the button 2 twice to enter the letter B, 
then pause for another second, and press the button 2 three times, to enter the letter C. You would have to press the button 2 six times in total.
...
def presses(phrase)
  hash = Hash.new(1)
  ['B', 'E', 'H', 'K', 'N', 'Q', 'U', 'X', '0'].each {|ch| hash[ch] = 2}
  ['C', 'F', 'I', 'L', 'O', 'R', 'V', 'Y'     ].each {|ch| hash[ch] = 3}
  ['S', 'Z', '2', '3', '4', '5', '6', '8'     ].each {|ch| hash[ch] = 4}
  hash[7] = hash[9] = 5
  (phrase.upcase.chars.map {|ch| hash[ch]} ).sum
end
